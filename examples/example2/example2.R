...
chunkfile<-"example2b_linked.chunkcounts.out" ## chromopainter chunkcounts file
mcmcfile<-"example2b_linked_mcmc.xml" ## finestructure mcmc file
treefile<-"example2b_linked_tree.xml" ## finestructure tree file
treefile_wh<-"example2b_linked_tree_wh.xml" ## finestructure tree file
...
system("fs fs -m T -k 2 example2b_linked.chunkcounts.out example2b_linked_mcmc.xml example2b_linked_tree_wh.xml")
...
## Where are good places to cut the tree?
theights<-uniqueCutHeights(tdend_wh)
theights[theights>0.01]

## Cut the tree to get K=3
tdend2<-cutdend(tdend_wh,theights["3"])
...
abline(h=theights[theights>0.01],col="grey")
abline(h=theights["3"],col=2)
text(length(labels(tdend))-1:sum(theights>0.01),theights[theights>0.01],paste0("k=",names(theights[theights>0.01])))
...
cutdendclear<-cutdend(tdend_wh,theights["3"],"NameMoreSummary")
